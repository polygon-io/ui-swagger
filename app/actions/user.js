import axios, { AxiosAccountServices } from './../helpers/axios';
...
export function fetchUser() {
	return (dispatch) => {
		...
		return AxiosAccountServices.get('accounts')
			.then((res) => {
			...
			.then(() => {
				return AxiosAccountServices.get('keys');
			})
			.then((res) => {
				return dispatch({
					type: 'UPDATE_USER',
					payload: {
						apiKey: res?.data?.results[0]?.key
					}
				});
			})
			.catch((err) => {
